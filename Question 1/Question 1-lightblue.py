...
class TranslationApp:
    def __init__(self, root):
        ...
        header = tk.Label(self.root, text="CDU-Group 305 \n Language Translation App", font=("Helvetica", 16, "bold"), bg='lightblue')   #edit for lightblue   ****
        header.pack(pady=20)
    ...
    def create_widgets(self):
        ...
        input_frame.configure(bg='lightblue')    #edit for lightblue       ******
        root.configure(bg='lightblue')              #edit for lightblue   ******

        # Label for input field
        tk.Label(input_frame, text="Enter text:", bg='lightblue').pack(side=tk.LEFT)   #edit for lightblue

        # Input box for text
        self.input_text = tk.Entry(input_frame, width=30)
        self.input_text.pack(side=tk.LEFT)
        self.input_text.configure(bg='lightblue')   #edit for lightblue   *******

        # Language selection frame
        lang_frame = tk.Frame(self.root, bg='lightblue')   #edit for lightblue   *******
        ...
        # Language dropdowns
        tk.Label(lang_frame, text="From:", bg='lightblue').pack(side=tk.LEFT)  #edit for lightblue     ******
        ...
        self.from_menu.configure(bg='lightblue')    #edit for lightblue     ******
        self.from_menu.pack(side=tk.LEFT)
        

        tk.Label(lang_frame, text="To:", bg='lightblue').pack(side=tk.LEFT)     #edit for lightblue   ****8
        self.to_menu = tk.OptionMenu(lang_frame, self.lang_to, *self.translator.languages)
        self.to_menu.configure(bg='lightblue')    #edit for lightblue     ******
        self.to_menu.pack(side=tk.LEFT)

        # Translate button
        tk.Button(self.root, text="Translate", command=self.perform_translation, bg='lightblue').pack(pady=20)  #edit for lightblue   ****

        # Label for output translation
        self.output_label = tk.Label(self.root, text="", fg="blue", bg='lightblue')   #edit for lightblue   ****
